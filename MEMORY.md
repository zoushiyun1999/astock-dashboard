...
| 2026-09-15 | header 状态项用 **`SHORT_NAME`** 映射（量价选股→量价、次日验证→验证、投资日历→日历），footer 仍用全称 | 全称在 430px 屏上合计 405px > 可用 398px → 折成两行，白占 18px。短名与 Tab 名一致，反而更好认 | 把 `srcMeta()` 里的 name 直接改成短名（footer 是完整句子，短名会读不通） |
