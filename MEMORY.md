...
- **🔴 无涨跌幅限制的次新股会污染验证统计**（2026-09-18 实测）：`601091 沈鼓集团` 是 09-17 上市的
  **C 前缀次新**（上市前 5 个交易日不设涨跌幅限制），09-18 由他日 20.80 元开盘 15.00 元、收 57.77 元，
  单只 **+285%**，把早报 9 只的实盘均值从 +1.76% 拉到 +33.16%。已用腾讯前复权日线独立核验
  （20.80/15.00/82.59/14.68/57.77 逐字段一致）→ **数据无误，是真实的次新无限制波动**。
  ⇒ **回测统计必须把这类标的单列或剔除**；`health_check.js` 的 ±21% 异常闸门已加豁免
  （读报告自述文字命中 `/无涨跌幅限制|不设.{0,2}涨跌幅/` 才跳过，声明缺失仍上报）。
- **`evening` 推荐股的字段路径是 `明日关注[].picks`，不是 `板块热点[].picks`**（2026-09-18 再次确认）：
  `板块热点` 是扁平摘要（`name/strength/stocks/catalyst`）、**根本没有 picks**。定时任务书里写成
  「板块热点 picks」会让人取错列表、静默得 0 条。两结构条数必须一致，但**只有 `明日关注` 带明细**。
...
- [x] ~~`verify.gain` 口径改造~~ → **2026-09-14 21:30 完成数据侧**：verify 增写
      ...
      **前端仍显示 `gain`（旧口径），待改 `verifyBadge()` 优先展示 `buyRet` / `netRet` 并给一字板独立标记**。
      （AGENTS 硬性规则 26）
      → **2026-09-14 前端也已完成**（`dashboard/js/app.js:458 verifyBadge()` 已是实盘口径优先、
      一字板独立标 🔒），此项**彻底闭环**，任务书里再出现「建议增加 verify 字段 / 前端改显示实盘口径」属过期。
- [x] ~~更新回测基线~~ → **2026-09-18 23:2x 重算（n=112，剔停牌 1 + 无涨跌幅限制次新 601091）**：
      平均高开 **+1.80%**、实盘毛 **−0.17%** / 净 **−0.37%**、净胜率 **42.9%**（n=99 时为
      +2.41% / −0.37% / −0.57%，方向一致）。**「人气榜推荐平均高开 ~2%，按开盘买入是负收益」为当前结论。**
      逐日明细：`.workbuddy/memory/DETAILS.md §1`。
