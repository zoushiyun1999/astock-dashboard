...
26. **「推荐效果验证」的口径必须写实盘，且 `at` 必须是行情日**。`verify` 字段固定为：
    ...
    { "gain": 1.36, "hit": true, "price": 15.63, "code": "600410", "at": "2026-09-14",
      ...
      "locked": false, "basis": "open-to-close" }
    ```
    ...
    - **奖励：历史缺口回补**。本任务只在 09-03/04/08 成功跑过，导致 09-07、09-09、09-10、
      09-11、09-14 共 **85 条推荐从未验证**（`verify.at` 的取值分布一眼可见）。回补口径：
      遍历所有 `date` 无 `verify` 的推荐，早报用当日行情、晚报用「该日之后最近一个有 reports 的交易日」行情。
